ladies_child =["トップス","ジャケット・アウター","パンツ","スカート","ワンピース","靴","ルームウェア・パジャマ","レッグウェア","帽子","バッグ","アクセサリー","ヘアアクセサリー","小物","時計","ウィッグ/エステ","浴衣/水着","スーツ/フォーマル/ドレス","マタニティ"]
ladies_grandchild_array = [
  ...
  ['テーラードジャケット','ノーカラージャケット','Gジャン/デニムジャケット','レザージャケット','テーラードジャケット','ノーカラージャケット','Gジャン/デニムジャケット','レザージャケット','ダウンベスト','ジャンパー/ブルゾン','ポンチョ','ロングコート','トレンチコート','ダッフルコート','ピーコート'],
  ...
  ['前髪ウィック','ロングストレート','ロングカール','ショートストレート','ショートカール','その他']
  ['浴衣','着物','振袖','長襦袢/半襦袢','水着セパレート','水着ワンピース','水着スポーツ用','その他']
  ['スカートパンツ上下','パンツスカート上下','ドレス','パーティーバック','シューズ','ウエディング',"その他"]
...
mens_child =["トップス","ジャケットアウター","パンツ","靴","バック","スーツ","帽子","アクセサリー","小物","時計","水着","レッグウエア","アンダーウエア","その他"]
mens_grandchild_array = [
  ['Tシャツ/カットソー(半袖/袖なし)','Tシャツ/カットソー(七分/長袖)','シャツ','ポロシャツ','タンクトップ','ニット/セーター','パーカー','カーディガン','スエット','ジャージ','ベスト','その他']
  ['テーラードジャケット','ノーカラージャケット','Gジャン/デニムジャケット','レザージャケット','ダウンベスト','ライダースジャケット','ミリタリージャケット','ナイロンジャケット','フライドジャケット','ダッフルコート','ピーコート','ステンカラーコート','トレンチコート','モッズコート','チェスターコート','スタジャン','スカジャン','ブルゾン','マウンテンパーカー','ダウンベスト','ポンチョ','カバーオール','その他']
  ['デニム/ジーンズ','ワークパンツ','スラックス','チノパン','ショートパンツ','ペインダーパンツ','サムエルパンツ','オーバーオール','その他']
  ['スニーカー','サンダル','ブーツ','モカシン','ドレス/ビジネス','長靴/レインシューズ','デッキシューズ','その他']
  ['ショルダーバック','トートバック','ボストンバック','リュック/バックパック','ウエストポーチ','ボディーパック','ドラムパック','ビジネスバック','トラベルバック','メッセンジャーバック','エコバック','その他']
  ['スーツ/ジャケット','スーツベスト','スラックス','セットアップ','その他']
  ['キャップ','ハット','ニットキャップ/ビーニー','ハンチング/ベレー帽','キャスケット','サンバイザー','その他']
  ['ネックレス','ブレスレット','ハングル/リストバンド','リング','ピアス(片耳用)','ピアス(両耳用)','アンクレット','その他']
...
mens_child_array.each_with_index do |child, i|
  child = parent.children.create(name: child)
  mans_grandchild_array[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end

baby_kids_child =["ベビー服(女の子用) ~95cm","ベビー服(男の子用) ~95cm","ベビー服(男女兼用) ~95cm","キッズ服(女の子用) 100cm~","キッズ服(男の子用) 100cm~","キッズ服(男女兼用) 100cm~","キッズ靴","子ども用ファッション小物","おむつ/トイレ/バス","外出/移動用品","授乳/食事","ベビー家具/寝具/室内用品","おもちゃ","行事/記念品","その他"]
...
interior_residence_accessory_child =["キッチン/食器","ベッド/マットレス","ソファ/ソファベッド","椅子/チェア","机/テーブル","収納家具","ラグ/カーペット/マット","カーテン/ブラインド","ライト/照明","寝具","インテリア小物","季節/年中行事""その他"]
interior_residence_accessory_grandchild_array = [
...
interior_residence_accessory_child_array.each_with_index do |child, i|
  child = parent.children.create(name: child)
  interior_residence_accessory_grandchild_array[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end

book_music_game_child =["本","漫画","CD","DVD/ブルーレイ","レコード","テレビゲーム"]
book_music_game_accessory_grandchild_array = [
...
toy_hobby_goods_child =["おもちゃ","タレントグッズ","コミック/アニメグッズ","トレーディングカード","フィギュア","楽器/器材","コレクション","ミリタリー","美術品","アート用品","その他"]
toy_hobby_goods_accessory_grandchild_array = [
...
cosme_perfume_beauty_child =["ベースメイク","メイクアップ","ネイルケア","香水","スキンケア/基礎化粧品","ヘアケア","ボディケア","オーラルケア","リラクゼーション","ダイエット","その他"]
cosme_perfume_beauty_accessory_grandchild_array = [
...
cosme_perfume_beauty_child_array.each_with_index do |child, i|
  child = parent.children.create(name: child)
...
appliance_smartphone_camera_child =["スマートフォン/携帯電話","スマホアクセサリー","PC/タブレット","カメラ","テレビ/映像機器,"オーディオ機器","美容/健康","美容/健康","生活家電","その他"]
appliance_smartphone_camera_accessory_grandchild_array = [
...
sport_leisure_child =["ゴルフ","フィッシング","自転車","トレーニング/エクササイズ","野球","サッカー/フットサル","テニス","スノーボード","スキー","その他スポーツ","アウトドア","その他"]
sport_leisure_accessory_grandchild_array = [
...
asport_leisure_child_array.each_with_index do |child, i|
  child = parent.children.create(name: child)
...
handmade_accessory_grandchild_array = [
...
ticket_accessory_grandchild_array = [
...
car_motorcycle_child =["自動車本体"',"自動車タイヤ/ホイール","自動車パーツ","自動車アクセサリー","オートバイ車体","オートバイパーツ","オートバイアクセサリー"]
car_motorcycle_accessory_grandchild_array = [
  ['国内自動車本体','外国自動車本体']
...
others_accessory_grandchild_array = [
  ['すべて']
